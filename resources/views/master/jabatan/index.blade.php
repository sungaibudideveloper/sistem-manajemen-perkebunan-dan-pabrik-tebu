<x-layout>
  ...
  >
    ...
    <div class="flex items-center justify-between px-4 py-2">
      {{-- Create Button --}}
      @if(auth()->user() && in_array('Create Jabatan', json_decode(auth()->user()->permissions ?? '[]')))
              ...
              <form method="POST"
                ...
                <div class="text-center sm:text-left">
                  <h3
                    ...
                    id="modal-title"
                    x-text="mode === 'edit' ? 'Edit Jabatan' : 'Create Jabatan'"
                  ></h3>
                  ...
                  <div class="mt-4 space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        name="namajabatan"
                        id="namajabatan"
                        x-model="form.namajabatan"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        maxlength="30"
                        required
                        x-init="form.namajabatan = '{{ old('namajabatan') }}'"
                      >
                      @error('namajabatan')
        ...
        <table class="table-fixed w-full bg-white text-sm text-center">
            <thead>
                <tr class="bg-gray-100 text-gray-700">
                    <th class="w-3/12 py-2 px-4 border-b">ID</th>
                    <th class="w-6/12 py-2 px-4 border-b">Nama Jabatan</th>
                    <th class="w-3/12 py-2 px-4 border-b">Aksi</th>
                </tr>
            </thead>
            <tbody>
            @foreach($jabatan as $i => $data)
              <tr class="hover:bg-gray-50">
                <td class="py-2 px-4 border-b">{{ $data->idjabatan }}</td>
                <td class="py-2 px-4 border-b">{{ $data->namajabatan }}</td>
                <td class="py-2 px-4 border-b">
                  <div class="flex items-center justify-center space-x-2">
                    {{-- Edit --}}
                    @if(auth()->user() && in_array('Edit Jabatan', json_decode(auth()->user()->permissions ?? '[]')))
                      <button
                        @click="
                          mode = 'edit';
                    ...
                    {{-- Delete --}}
                    @if(auth()->user() && in_array('Hapus Jabatan', json_decode(auth()->user()->permissions ?? '[]')))
                      <form
                        action="{{ url("masterdata/jabatan/{$data->idjabatan}") }}"
                        method="POST"
    ...
    <div class="mx-4 my-1">
      @if($jabatan->hasPages())
        {{ $jabatan->appends(['perPage' => $jabatan->perPage(), 'search' => $search])->links() }}
      @else
        <div class="flex items-center justify-between">
          <p class="text-sm text-gray-700">
            Showing <span class="font-medium">{{ $jabatan->count() }}</span> of <span class="font-medium">{{ $jabatan->total() }}</span> results
          </p>
        </div>
      @endif
    </div>
