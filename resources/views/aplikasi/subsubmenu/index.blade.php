    ...
    <div x-data="{
        ...
        <div class="mx-auto px-4 py-4">
            <div class="overflow-x-auto rounded-md border border-gray-300">
                <table class="min-w-full bg-white text-sm text-center">
                    <thead>
                        <tr>
                            <th class="py-2 px-4 border-b bg-gray-100">Submenu Group</th>
                    ...
                    <tbody>
                        @foreach ($data as $d)
                        <tr>
                            <td class="py-2 px-4 border-b">{{ $d->submenu_name }}</td>
        ...
        <div x-show="open" x-cloak class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
            ...
            <div class="bg-white rounded-md p-6 w-96 max-w-full shadow-lg">
                ...
                <form :action="mode === 'create' ? '{{ route('aplikasi.subsubmenu.store') }}' : '{{ url('aplikasi/subsubmenu') }}/' + form.subsubmenuid" method="POST">
                    @csrf
                    ...
                    {{-- Menu ID (Dropdown) --}}
                    <div class="mb-4">
                        <label for="menuid" class="block text-sm font-medium text-gray-700">Submenu Group</label>
                        <select id="menuid" name="submenuid" x-model="form.submenuid" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring focus:ring-blue-500">
                            <option value="">-- Pilih Menu --</option>
                            @foreach ($allMenu as $a)
                                <option value="{{ $a->submenuid }}">{{ $a->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    {{-- Submenu Name --}}
                    <div class="mb-4">
                        <label for="submenuname" class="block text-sm font-medium text-gray-700">Nama Subsubmenu</label>
                        <input type="text" id="submenuname" name="subsubmenuname" x-model="form.submenuname" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring focus:ring-blue-500" />
                    </div>
