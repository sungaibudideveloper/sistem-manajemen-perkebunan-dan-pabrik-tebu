    ...
    <script>
        ...
        function mainData() {
            return {
                ...
                showApprovalDetailModal: false, // NEW
                ...
                pendingApprovals: [],
                userInfo: {},
                selectedRKHs: [], // NEW: untuk bulk selection
                approvalDetail: {}, // NEW: untuk detail modal
                ...
                async loadPendingApprovals() {
                    try {
                        ...
                        if (data.success) {
                            ...
                            this.selectedRKHs = []; // Reset selection
                        } else {
                ...
                // UPDATED: Individual approve (masih bisa dipakai untuk bulk)
                async approveRKH(rkhno, level) {
                    const result = await this.processApproval(rkhno, 'approve', level);
                    if (result.success) {
                        alert(result.message);
                        await this.loadPendingApprovals();
                        location.reload();
                    } else {
                        alert('Gagal approve RKH: ' + result.message);
                    }
                },

                // UPDATED: Individual decline (masih bisa dipakai untuk bulk)
                async declineRKH(rkhno, level) {
                    const result = await this.processApproval(rkhno, 'decline', level);
                    if (result.success) {
                        alert(result.message);
                        await this.loadPendingApprovals();
                        location.reload();
                    } else {
                        alert('Gagal decline RKH: ' + result.message);
                    }
                },

                // NEW: Helper function for processing approval
                ...
                // Get approval level class for styling
                ...
                // NEW: Clear approval detail saat modal ditutup
                clearApprovalDetail() {
                    this.approvalDetail = {};
                    this.selectedRkhno = '';
                }
