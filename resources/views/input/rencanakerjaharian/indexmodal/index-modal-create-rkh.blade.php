...
function createRkhModal() {
    return {
        ...
        async proceedToCreate() {
            ...
            try {
                const response = await fetch('{{ route("input.rencanakerjaharian.checkOutstanding") }}', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success && !data.hasOutstanding) {
                    ...
                    window.location.href = `{{ route('input.rencanakerjaharian.create') }}?date=${this.selectedDate}&mandor_id=${this.selectedMandor.userid}`;
