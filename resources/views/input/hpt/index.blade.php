<x-layout>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        @if (auth()->user() && in_array('Edit HPT', json_decode(auth()->user()->permissions ?? '[]')))
                                            @if ($item->status === 'Unposted')
                                                <a href="{{ route('input.hpt.edit', ['no_sample' => $item->no_sample, 'kd_comp' => $item->kd_comp, 'tgltanam' => $item->tgltanam]) }}"
                                        ...
                                        @if (auth()->user() && in_array('Hapus HPT', json_decode(auth()->user()->permissions ?? '[]')))
                                            @if ($item->status === 'Unposted')
                                                <form
                                                    action="{{ route('input.hpt.destroy', ['no_sample' => $item->no_sample, 'kd_comp' => $item->kd_comp, 'tgltanam' => $item->tgltanam]) }}"
    ...
    <script>
        function showList(no_sample, kd_comp, tgltanam) {
            ...
            const url =
                `{{ route('input.hpt.show', ['no_sample' => '__no_sample__', 'kd_comp' => '__kd_comp__', 'tgltanam' => '__tgltanam__']) }}`
