<x-layout>
  ...
  <div 
    x-data="{
      ...
      mode: 'create',
      form: { kodekategori: '', namakategori: '' },
      resetForm() {
        this.mode = 'create';
        this.form = { kodekategori: '', namakategori: '' };
        this.open = true;
      }
    }"
  ...
  >
    ...
    <div class="flex items-center justify-between px-4 py-2">
        {{-- Create Button --}}
      @can('masterdata.kategori.create')
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
          <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div x-show="open" x-transition class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl sm:my-8 sm:w-full sm:max-w-lg">
              <form method="POST"
                    :action="mode === 'edit'
                      ? '{{ url('masterdata/kategori') }}/' + form.kodekategoriOriginal
                      : '{{ url('masterdata/kategori') }}'"
                ...
                <h3 class="text-lg font-medium text-gray-900" x-text="mode === 'edit' ? 'Edit Kategori' : 'Create Kategori'"></h3>

                <div class="mt-4 space-y-4">
                  <div>
                    <label for="kodekategori" class="block text-sm font-medium text-gray-700">Kode Kategori</label>
                    <input
                      type="text"
                      name="kodekategori"
                      id="kodekategori"
                      x-model="form.kodekategori"
                      class="mt-1 block w-1/2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                      maxlength="3"
                      required
                      x-init="form.kodekategori = '{{ old('kodekategori') }}'"
                    >
                    @error('kodekategori')
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="namakategori"
                      id="namakategori"
                      x-model="form.namakategori"
                      class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                      maxlength="30"
                      required
                      x-init="form.namakategori = '{{ old('namakategori') }}'"
                    >
                    @error('namakategori')
                      <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
        ...
        <table class="table-fixed w-full bg-white text-sm text-center">
          <thead>
            <tr class="bg-gray-100 text-gray-700">
              ...
              <th class="w-6/12 py-2 px-4 border-b">Nama Kategori</th>
          ...
          <tbody>
            @foreach($kategori as $data)
              <tr class="hover:bg-gray-50">
                <td class="py-2 px-4 border-b">{{ $data->kodekategori }}</td>
                <td class="py-2 px-4 border-b">{{ $data->namakategori }}</td>
                <td class="py-2 px-4 border-b">
                  <div class="flex items-center justify-center space-x-2">
                    @can('masterdata.kategori.edit')
                      <button @click="
                        mode = 'edit';
                        form.kodekategoriOriginal = '{{ $data->kodekategori }}';
                        form.kodekategori = '{{ $data->kodekategori }}';
                        form.namakategori = '{{ $data->namakategori }}';
                        open = true;
                      "
                    ...
                    @endcan
                    @can('masterdata.kategori.delete')
    ...
    <div class="mx-4 my-1">
      @if($kategori->hasPages())
        {{ $kategori->appends(['perPage' => $kategori->perPage(), 'search' => $search])->links() }}
      @else
        <div class="flex items-center justify-between">
          <p class="text-sm text-gray-700">
            Showing <span class="font-medium">{{ $kategori->count() }}</span> of <span class="font-medium">{{ $kategori->total() }}</span> results
          </p>
        </div>
      @endif
    </div>
