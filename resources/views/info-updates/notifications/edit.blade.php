{{-- resources/views/info-updates/notifications/edit.blade.php --}}
<x-layout>
    <x-slot:title>{{ $title }}</x-slot:title>
    <x-slot:navbar>Info & Updates</x-slot:navbar>
    <x-slot:nav>Edit Notification</x-slot:nav>

    <div class="mx-auto py-4 bg-white rounded-md shadow-md">
        <div class="px-6 py-4">
            <form action="{{ route('info-updates.notifications.admin.update', $notification->notification_id) }}" method="POST" x-data="notificationForm()">
                @csrf
                @method('PUT')
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <p class="text-sm text-gray-700">
                                Editing notification created on <strong>{{ $notification->createdat->format('d M Y H:i') }}</strong>
                            </p>
                            <p class="text-xs text-gray-500 mt-1">
                                Type: <span class="font-medium">{{ ucfirst($notification->notification_type) }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="relative">
                        ...
                        <div x-show="showJabatanDropdown" @click.away="showJabatanDropdown = false" x-cloak
                            ...
                            <div class="p-3 space-y-2">
                                @php
                                    $existingJabatan = $notification->target_jabatan ? explode(',', $notification->target_jabatan) : [];
                                @endphp
                                @foreach ($jabatan as $jab)
                                <label class="flex items-center space-x-3 p-2 hover:bg-gray-50 rounded cursor-pointer">
                                    <input type="checkbox" name="target_jabatan[]" value="{{ $jab->idjabatan }}"
                                        x-model="selectedJabatan"
                                        {{ in_array($jab->idjabatan, $existingJabatan) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="body" rows="5" required
                        ...
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('body') border-red-500 @enderror">{{ old('body', $notification->body) }}</textarea>
                    @error('body')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="url" name="action_url" value="{{ old('action_url', $notification->action_url) }}"
    ...
    function notificationForm() {
        return {
            ...
            selectedCompanies: {{ json_encode(explode(',', $notification->companycode)) }},
            selectedJabatan: {{ json_encode($notification->target_jabatan ? explode(',', $notification->target_jabatan) : []) }},
